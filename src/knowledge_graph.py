...
class KnowledgeGraph(nn.Module):
    ...
    def load_graph_data(self, data_dir):
        ...
        if self.args.model.startswith('point'): 
            ...
            227: {264}, 4: {191, 612, 901, 486, 424, 1065, 874, 457, 1068, 1165, 238, 813, 1361, 1363, 1493, 759, 575}, 
            18: {612, 901, 934, 486, 424, 1065, 841, 457, 1068, 874, 1165, 1363, 759, 636, 1117, 575}, 
            3: {612, 901, 806, 934, 486, 841, 1065, 874, 1068, 813, 1165, 238, 575, 1361, 1363, 1493, 1117, 191}, 
            416: {7201}, 97: {3803, 486, 1259}, 98: {1259, 3803, 486}, 31: {24}, 7: {243, 44, 245, 255}, 403: {1259}, 
            48: {1918}, 21: {4097, 346, 1667, 239}, 35: {1016, 17}, 26: {360}, 94: {2966}, 61: {17}, 
            311: {1066}, 312: {1066, 484}, 74: {2689, 9562, 4903, 7503}, 56: {239}, 93: {1066, 2966}, 289: {3273}, 
            39: {5}, 404: {1259}, 9: {8, 343}, 228: {264}, 325: {1259}, 29: {2}}), 
            """
    ...
    def vectorize_action_space(self, data_dir):
        ...
        def get_action_space(e1):
            # 获得 entity e1 的action space [(r, e2)]
            action_space = []
            if e1 in self.adj_list:
                ...
                if len(action_space) + 1 >= self.bandwidth:
                    # Base graph pruning 如果大于 bandwidth ，进行剪枝
                    sorted_action_space = \
        ...
        def get_unique_r_space(e1):
            if e1 in self.adj_list:
                return list(self.adj_list[e1].keys())       # 返回adj_list 中的 e1 存在的关系 [r]
            else:
                return []

        def vectorize_action_space(action_space_list, action_space_size):
            # 对 action space 进行初始化
